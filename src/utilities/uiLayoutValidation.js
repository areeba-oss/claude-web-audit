  ...
  try {
    ...
    const structure = await page.evaluate(() => {
      ...
      const header = document.querySelector(
        'header,[role="banner"],#header,.header,.site-header,.navbar',
      );
    ...
    if (cta) {
      ...
      const fb = await page.evaluate(() => {
        ...
        return {
          count: els.length,
          texts: els.slice(0, 5).map((el) => (el.innerText || el.value || '').trim()),
        };
      });
    ...
    const logoData = await page.evaluate((origin) => {
      const candidates = Array.from(
        document.querySelectorAll(
          'header img,header svg,[class*="logo"],[id*="logo"],.brand img,.navbar-brand img',
        ),
      );
      let best = null,
        bestScore = -1;
      for (const el of candidates) {
        ...
        const visible =
          box.width > 0 && box.height > 0 && window.getComputedStyle(el).display !== 'none';
        if (visible) score += 1;
        const href = el.closest('a')?.href || null;
        if (href?.startsWith(origin)) score += 2;
        if (score > bestScore) {
          bestScore = score;
          best = { visible, href, width: box.width, height: box.height };
        }
      }
      return best;
    }, origin);
