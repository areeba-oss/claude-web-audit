...
const basicHealthCheck = require('./utilities/basicHealthCheck');
const uiLayoutValidation = require('./utilities/uiLayoutValidation');
const { getPageSnapshot, analyzePageWithAI } = require('./utilities/aiDetector');

// Audits
const formsAudit = require('./audits/formsAudit');
const ecommerceAudit = require('./audits/ecommerceAudit');
const performanceAudit = require('./audits/performanceAudit');
const navigationLinksAudit = require('./audits/navigationLinksAudit');
  ...
  try {
    ...
    const linkValidationPromise = (async () => {
      ...
      const [iRes, eRes] = await Promise.all([
        pLimit(
          internal.map((h) => () => validate(h)),
          3,
        ),
        pLimit(
          external.map((h) => () => validate(h)),
          5,
        ),
      ]);
      const cache = new Map();
    ...
    if (aiResult?.links) {
      ...
      if (aiBroken.length === 0 && rawBroken.length > 0) {
        aiResult.links.genuinelyBroken = rawBroken.filter(
          (l) => l.status === 404 || l.status >= 500,
        );
        aiResult.links.falsePositives = rawBroken.filter(
          (l) => l.status === 401 || l.status === 403,
        );
      }
    }
    ...
    console.log(
      `     ✅ Done [${method}] health:${health.aiHealthScore} perf:${performance.performanceScore} links:${navigation.summary?.genuinelyBroken ?? 0} broken`,
    );
...
function printSummary(results) {
  ...
  const t = {
    ...
    blankScreens: 0,
    significantErrors: 0,
    criticalFailedReqs: 0,
    missingHeaders: 0,
    missingFooters: 0,
    missingCTAs: 0,
    missingLogos: 0,
    responsiveIssues: 0,
    genuinelyBrokenLinks: 0,
    formIssues: 0,
    ecommercePages: 0,
    ecommerceIssues: 0,
    poorPerformance: 0,
  ...
  for (const r of results) {
    ...
    if (!r.health?.httpOk) {
      issues.push(`HTTP ${r.health?.httpStatus}`);
      t.httpErrors++;
    }
    if (r.health?.blankScreen) {
      issues.push('Blank screen');
      t.blankScreens++;
    }
    ...
    if ((r.health?.criticalFailedCount || 0) > 0) {
      issues.push(`${r.health.criticalFailedCount} critical failed req(s)`);
      t.criticalFailedReqs += r.health.criticalFailedCount;
    }

    if (!r.uiLayout?.headerVisible) {
      issues.push('Header not visible');
      t.missingHeaders++;
    }
    if (!r.uiLayout?.footerVisible) {
      issues.push('Footer not visible');
      t.missingFooters++;
    }
    if (!r.uiLayout?.mainCTAVisible) {
      issues.push('No CTA found');
      t.missingCTAs++;
    }
    if (!r.uiLayout?.logoVisible) {
      issues.push('Logo not visible');
      t.missingLogos++;
    }

    const respIssues = Object.entries(r.uiLayout?.responsiveness || {})
      .filter(([, v]) => !v.responsive)
      .map(([k]) => k);
    if (respIssues.length) {
      issues.push(`Not responsive: ${respIssues.join(', ')}`);
      t.responsiveIssues++;
    }
    ...
    if (broken > 0) {
      issues.push(`${broken} broken link(s)${fps ? ` (${fps} false+ excluded)` : ''}`);
      t.genuinelyBrokenLinks += broken;
    }

    (r.forms?.forms || [])
      .filter((f) => f.isCritical && f.criticalMissingValidation?.length)
      .forEach((f) => {
        issues.push(`Form [${f.purpose}]: missing ${f.criticalMissingValidation.join(', ')}`);
        t.formIssues++;
      });

    if (r.ecommerce?.isEcommerce) {
      t.ecommercePages++;
      if (!r.ecommerce.addToCartWorked) {
    ...
    if (perf === 'poor' || perf === 'needs-improvement') {
      issues.push(
        `Perf: ${perf} — FCP ${r.performance?.firstContentfulPaint}ms TTFB ${r.performance?.timeToFirstByte}ms`,
      );
      t.poorPerformance++;
  ...
  const ai = AI_ENABLED
    ? '✅ ON — Gemini 2.5 Flash-Lite (free)'
    : '⚠️  OFF — set GEMINI_API_KEY in .env';
  console.log('\n  ────────────────────────────────────────────────────');
  console.log(`  AI              : ${ai}`);
...
async function runAudit(inputUrl, maxPages = 25) {
  ...
  console.log(`\n  🌐 URL    : ${inputUrl}`);
  console.log(
    `  🤖 AI     : ${AI_ENABLED ? 'ON — 1 batched call/page' : 'OFF — set GEMINI_API_KEY in .env'}`,
  );
