...
async function getPageSnapshot(page) {
    return await page.evaluate(() => {
        ...
        document.querySelectorAll('h1,h2,h3').forEach(h =>
            parts.push(`${h.tagName}: ${h.innerText?.trim().slice(0, 100)}`)
        );
        document.querySelectorAll('button,input[type="submit"],input[type="button"],a[role="button"]').forEach(el =>
            parts.push(`BUTTON: ${(el.innerText || el.value || el.getAttribute('aria-label') || '').trim().slice(0, 80)}`)
        );
        Array.from(document.querySelectorAll('a[href]')).slice(0, 50).forEach(a =>
            parts.push(`LINK: ${a.innerText?.trim().slice(0, 50)} → ${a.href}`)
        );
        Array.from(document.querySelectorAll('p,li,[class*="price"],[itemprop="price"]')).slice(0, 15).forEach(p => {
            const t = p.innerText?.trim();
            if (t?.length > 10) parts.push(`TEXT: ${t.slice(0, 120)}`);
        });
        Array.from(document.querySelectorAll('form')).forEach((form, i) => {
            const fields = Array.from(form.querySelectorAll('input,textarea,select'))
                .map(f => `${f.type||'text'}[${f.name||f.placeholder||''}]`).join(', ');
            const submit = form.querySelector('[type="submit"],button:not([type])')?.innerText?.trim() || '';
...
async function analyzePageWithAI({ snapshot, consoleErrors, consoleWarnings, failedRequests, httpStatus, perfMetrics, brokenLinks, url }) {
...
Return ONLY valid JSON with this exact structure:
{
  ...
  "performance": {
    ...
    const noisePatterns = [/google-analytics/, /googletagmanager/, /pagead/, /doubleclick/, /facebook\.net/, /hotjar/];
...
Failed Requests: ${JSON.stringify(failedRequests.filter(r => !isNoise(r.url)).slice(0, 8).map(r => ({ url: r.url?.slice(0, 80), f: r.failure })))}
...
${snapshot.slice(0, 3000)}`;
    ...
    try {
        ...
        return {
            ...
            links: { genuinelyBroken: brokenLinks.filter(l => l.status === 404), falsePositives: [], severity: 'low', summary: '' },
            forms: [],
            ecommerce: { isEcommerce: false, confidence: 'low', type: 'none', productLinks: [], reasoning: err.message },
            performance: {
                overallScore: (perfMetrics.fcp ?? 9999) > 3000 ? 'poor' : (perfMetrics.fcp ?? 9999) > 1800 ? 'needs-improvement' : 'good',
                fcpAssessment: 'unknown', ttfbAssessment: 'unknown',
                topIssues: [], recommendations: [], userImpact: ''
            }
        };
    }
}
