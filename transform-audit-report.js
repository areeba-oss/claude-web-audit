/**
 * Web Audit Report Transformer
 * Converts raw Playwright audit JSON into a marketing-friendly report structure.
 * 
 * Usage: node transform-audit-report.js <input.json> [output.json]
 */
...
function scoreLabel(score) {
  ...
  if (score === "needs_improvement") return { label: "Needs Improvement", emoji: "⚠️", color: "#f59e0b" };
...
function computeOverallScore(page) {
  ...
  // Health
  if (page.health?.httpOk) { total += 100; count++; }
  else { total += 0; count++; }

  // Performance
  const fcp = page.performance?.firstContentfulPaint;
  if (fcp) { total += fcp < 1800 ? 100 : fcp < 3000 ? 65 : 30; count++; }

  const ttfb = page.performance?.timeToFirstByte;
  if (ttfb) { total += ttfb < 200 ? 100 : ttfb < 500 ? 70 : 30; count++; }

  // Mobile
  const mobile = page.uiLayout?.responsiveness?.mobile;
  if (mobile) { total += mobile.responsive && !mobile.hasHorizontalScroll ? 100 : 20; count++; }

  // Navigation
  if (page.navigation?.navigationHealth === "good") { total += 100; count++; }
  else if (page.navigation?.navigationHealth === "issues") { total += 70; count++; }
  else { total += 30; count++; }

  // No console errors bonus
  if (page.health?.consoleErrorCount === 0) { total += 100; count++; }
  else { total += Math.max(0, 100 - page.health.consoleErrorCount * 20); count++; }
...
function transformPage(raw) {
  ...
  if (health.consoleErrors > 0) findings.push({ type: "warning", message: `${health.consoleErrors} console error(s) detected — may indicate broken functionality.` });
  if (fcpMs && fcpMs >= 3000) findings.push({ type: "warning", message: `Slow page paint (${msToSeconds(fcpMs)}) — users may perceive the page as sluggish.` });
  if (ttfbMs && ttfbMs >= 500) findings.push({ type: "warning", message: `High server response time (${msToSeconds(ttfbMs)}) — server may need optimisation.` });
  if (resp.mobile && !resp.mobile.responsive) findings.push({ type: "critical", message: "Page is NOT mobile-responsive — failing on the majority of today's web traffic." });
  if (nav.brokenLinks?.length > 0) findings.push({ type: "critical", message: `${nav.brokenLinks.length} broken link(s) found — damages SEO and user trust.` });
  if (nav.navigationIssues?.length > 0) findings.push({ type: "info", message: `${nav.navigationIssues.length} navigation anchor link(s) have unknown resolution.` });
  if (!layout.ctaCount) findings.push({ type: "info", message: "No clear call-to-action buttons detected — missed conversion opportunity." });
  if (perf.slowResources?.length > 0) opportunities.push(`Optimise ${perf.slowResources.length} slow-loading resource(s) to improve page speed.`);
  if (!resp.mobile?.responsive) opportunities.push("Implement responsive CSS breakpoints for mobile devices (375px viewport).");
  if (!layout.ctaCount) opportunities.push("Add prominent CTA buttons to drive user engagement and conversions.");
  if (nav.navigationIssues?.length > 0) opportunities.push("Audit and fix anchor-based navigation links.");

  if (findings.length === 0) findings.push({ type: "success", message: "No critical issues detected on this page." });
...
function buildSummary(pages) {
  ...
  const avgFcp = Math.round(
    pages.filter((p) => p.performance.firstContentfulPaint.ms).reduce((a, p) => a + p.performance.firstContentfulPaint.ms, 0) /
      pages.filter((p) => p.performance.firstContentfulPaint.ms).length
  );

  const topFindings = [];
  if (criticalCount > 0) topFindings.push(`🚨 ${criticalCount} critical issue(s) found across ${pages.length} pages`);
  if (mobileFailCount > 0) topFindings.push(`📱 ${mobileFailCount} of ${pages.length} pages are not mobile-ready`);
  if (warningCount > 0) topFindings.push(`⚠️ ${warningCount} warning(s) flagged for review`);
  if (topFindings.length === 0) topFindings.push("✅ No critical issues detected across audited pages");
  ...
  return {
    ...
    topFindings,
    allOpportunities,
    pageScores: pages.map((p) => ({ url: p.url, slug: p.slug, score: p.overallScore, grade: p.grade })),
  };
}
...
function buildReport(rawPages) {
  ...
  return {
    ...
    executiveSummary: {
      headline: summary.grade.score >= 75
      ...
      keyStats: [
        ...
        { label: "Critical Issues", value: summary.criticalIssues, icon: "🚨", highlight: summary.criticalIssues > 0 },
        { label: "Warnings", value: summary.warnings, icon: "⚠️" },
        { label: "Mobile Failures", value: summary.mobileFailures, icon: "📱", highlight: summary.mobileFailures > 0 },
        { label: "Avg. Load Speed", value: summary.averageFirstContentfulPaint.formatted, icon: "⚡", subLabel: summary.averageFirstContentfulPaint.grade?.label },
      ],
      topFindings: summary.topFindings,
      callToAction: "The issues identified below are actionable improvements that can increase search rankings, improve user experience, and drive more conversions. Our team can resolve these efficiently.",
    },
      ...
      categories: [
        {
          ...
          score: Math.round(transformed.reduce((a, p) => a + (p.health.ok ? 100 : 0), 0) / transformed.length),
        },
        {
          name: "Page Speed",
        ...
        },
        {
          ...
          score: Math.round(transformed.reduce((a, p) => a + p.responsiveness.mobile.score, 0) / transformed.length),
        },
        {
          name: "Navigation & Links",
          description: "Broken links, anchor integrity, and link structure",
          score: Math.round(
            transformed.reduce((a, p) => a + (p.navigation.health.label === "All Links Working" ? 100 : 65), 0) / transformed.length
          ),
        },
        {
          name: "Layout & UX",
...
if (!inputPath) {
  console.error("Usage: node transform-audit-report.js <input.json> [output.json]");
  process.exit(1);
}
...
const outFile = outputPath ?? inputPath.replace(".json", "-report.json");

fs.writeFileSync(outFile, JSON.stringify(report, null, 2));
console.log(`✅  Report written to: ${outFile}`);
console.log(`📊  Pages processed: ${report.meta.auditedPages}`);
console.log(`🏆  Overall score: ${report.executiveSummary.overallScore}/100 (${report.executiveSummary.grade.grade})`);