...
async function fetchAllPages(homepage, maxPages = 25) {
    ...
    if (start) { queue.push(start); queued.add(start); }
        ...
        try {
            while (true) {
                ...
                try {
                    ...
                    if (!res || res.status() >= 400) { stats.skipped++; continue; }
                    stats.visited++;
                    ...
                    for (const link of links) {
                        ...
                        if (!norm) continue;
                        if (norm.startsWith(origin) && !visited.has(norm) && !queued.has(norm) && visited.size + queued.size < maxPages * 2) {
                            queued.add(norm);
                            queue.push(norm);
                        }
                    }
