...
async function ecommerceAudit(browser, page, url, aiResult) {
    ...
    if (!candidates.length) { result.notes.push('No product links found.'); return result; }
    ...
    try {
        ...
        const selectors = [
            ...
            'button[class*="add"]', 'button[class*="cart"]', 'button[class*="buy"]',
        ...
        for (const sel of selectors) {
            ...
            if (el && await el.isVisible().catch(() => false)) { addBtn = el; break; }
        }

        if (!addBtn) { result.notes.push('Add to cart button not found.'); return result; }
        ...
        const cartLink = await productPage.evaluate(origin => {
            const a = Array.from(document.querySelectorAll('a[href]'))
                .find(a => /cart|basket|bag/i.test(a.href) || /cart|basket|bag/i.test(a.innerText || ''));
            return a?.href || null;
        }, origin);
        ...
        try {
            ...
            const checkoutEl = await productPage.evaluate(() => {
                const el = Array.from(document.querySelectorAll('a[href],button'))
                    .find(el => /checkout|proceed|place.?order|pay.?now/i.test(el.innerText || '') || /checkout/i.test(el.getAttribute('href') || ''));
                return el ? (el.href || 'button') : null;
            });
            if (checkoutEl) { result.checkoutAccessible = true; result.checkoutPageUrl = checkoutEl; }
        } catch { result.notes.push('Cart page not reachable.'); }
